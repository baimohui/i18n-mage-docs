...
- 支持的服务包括:DeepL、Google、ChatGPT、DeepSeek、百度、腾讯、有道  
...
**Key 命名风格**  
- 配置 `i18n-mage.writeRules.generatedKeyStyle` 来设置生成的 key 命名风格
- 支持：小驼峰 (camelCase)、大驼峰 (PascalCase)、下划线 (snake_case)、短横线 (kebab-case)、原样 (raw)  

**Key 生成策略**
- 配置 `i18n-mage.writeRules.generatedKeyStrategy` 来选择 key 名称生成方式
...
**Key 最大长度**  
- 配置 `i18n-mage.writeRules.maxGeneratedKeyLength` 来限制生成 key 的最大长度
- 超长时会使用"文件名 + 文本 + 序号"的格式自动优化  
...
1. **在使用信息面板中操作**
   ...
   - 右键点击某个未定义词条或整个类别，选择"提取未定义文本"  

2. **修复执行**
   - 插件会仅处理选中的未定义词条  
   - 如果选中的是特定词条，会通过 `fixQuery.entriesToGen` 来限定处理范围  
