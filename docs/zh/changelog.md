...
### 新增
- **持久化搜索输入框**：重新设计了词条搜索界面，从触发式搜索（按回车搜索）改为持久化搜索输入框，支持实时过滤。包含搜索历史管理、大小写/全词匹配开关、键盘导航（F4/Shift+F4）以及 QuickPick 集成，实现更快速的访问。
- **引号剥离**：增强引用和导航功能，自动剥离选中范围的引号，实现更精确的键值匹配。

### 修复
- **动态词条搜索导航**：修复搜索动态词条（指向多个定义）时的结果导航问题，确保每个词条只停止一次，与普通词条行为一致。
- **忽略翻译目录中的子文件**：新增对翻译目录内子文件/子目录的忽略支持。
- **仅源语言搜索**：修复仅搜索源语言文案时，仅匹配源语言文本，排除词条 key。
- **大文件检测逻辑**：修复大文件检测逻辑，使其仅适用于国际化相关路径。

## [1.3.6] - 2026-04-14

### 新增
- **多工作区模式支持**：现已完整支持 VS Code 多工作区模式，可在多个根目录下同时使用插件功能。
- **搜索快捷导航**：搜索词条时支持快速跳转浏览搜索结果。支持 F4/Shift+F4 在全局结果中上下跳转，F3/Shift+F3 在当前文件结果中上下跳转，Ctrl+Tab/Ctrl+Shift+Tab 在不同文件间跳转。搜索视图同步添加了对应的操作按钮。
- **硬编码提取 AI 前缀配置**：首次提取硬编码文案时，支持配置使用 AI 选择 key 前缀，并开放前缀候选列表配置项。

### 修复
- **动态词条浏览支持**：修正浏览当前文件翻译功能，现已支持正常浏览动态拼接 key 相关的词条内容。
- **硬编码空文件生成问题**：解决从硬编码配置界面跳转到提取确认界面时，会意外直接生成空语言文件的问题。
- **非源语言 AI 前缀选择**：修复修正非源语言的未定义词条时，新生成的词条 key 无法应用 AI 自动选择前缀功能的问题。

## [1.3.5] - 2026-04-02

### 新增
- **资源管理器词条聚合菜单**：在资源管理器中对文件/目录右键菜单新增“词条管理”聚合操作项，下设复制词条、导出词条、提取未定义文案、提取硬编码文案四个快捷操作。
- **语言文件装饰增强**：为语言文件中的翻译内容添加内联装饰和悬浮信息展示，提升翻译文件的浏览体验。
- **AI 前缀选择上下文增强**：生成新 key 时，若使用 AI 选择 key 前缀，除文案外还将传入文案所在文件路径，为 AI 提供更充分的上下文信息以做出更适配的选择。
- **文件翻译浏览增强**：运行浏览当前文件翻译时，除原有翻译内容外，新增 key 展示，搜索功能同步支持对 key 进行匹配。
- **搜索结果批量复制**：词条搜索功能支持批量复制全部已定义词条、全部未定义词条或全部搜索结果数据。
- **源语言文案筛选**：词条搜索功能新增仅匹配源语言文案的筛选选项。

### 修复
- **预览编辑键名残留问题**：修复在修复预览页面编辑键名并应用后，编辑前的旧键名仍被写入翻译源语言文件的问题。

## [1.3.4] - 2026-03-25

### 新增
- **单语言重译支持**：支持对单个词条的单个非源语言文案进行独立重译，提供更精细的翻译调整能力。
- **统一忽略路径配置**：新增 `i18n-mage.workspace.ignoredPaths` 配置，支持同时指定文件和目录路径，统一管理忽略规则，替代原有的 `ignoredFiles` 和 `ignoredDirectories` 配置。
- **智能路径忽略命令**：新增 `i18nMage.toggleIgnorePath` 命令，支持在资源管理器中对文件或目录右键快速忽略/取消忽略，并为已忽略路径添加视觉装饰和悬浮提示，替代原有的 `ignoreFile` 和 `ignoreDirectory` 命令。
- **预览范围精细化**：新增 `i18n-mage.general.previewChangeScopes` 配置，支持选择需要预览的操作类型，替代原有的 `previewChanges` 配置。
- **词条粘贴预览**：新增词条数据粘贴预览功能，在写入前可预览待粘贴内容。

### 优化
- **变更预览界面优化**：优化变更预览界面的标签分类，提升信息层次感与操作清晰度。
- **词条粘贴智能去重**：优化词条粘贴逻辑，自动跳过与现有数据重复的内容，避免冗余写入。
- **版本差异导出增强**：导出 Excel 版本差异表时，可选 Git commit 数量由 80 条提升至 100 条，并在表格 README 说明中补充仓库 Git 地址信息。

### 修复
- **预览数据残留问题**：修复进入变更预览界面后未应用更改直接关闭时，修复数据未被清除导致下次进入时仍显示的问题。

## [1.3.3] - 2026-03-16

### 新增
- **更多 AI 服务支持**：新增千问、混元和 Kimi AI 翻译服务，进一步扩展翻译服务生态。
- **自定义 AI 服务**：支持配置与 OpenAI 兼容的自定义 AI 服务，满足个性化接入需求。
- **模型可配置化**：OpenAI 和 DeepSeek 服务现支持配置具体模型，提供更灵活的调用选择。
- **只读模式**：新增只读模式配置（默认关闭），启用后将隐藏写入相关操作入口并阻止写入命令执行。
- **版本更新摘要**：新增版本更新弹窗提醒，Grimorie 视图三点菜单同步添加更新日志入口。

### 优化
- **预览界面信息精简**：优化修复预览界面展示，将重复展示的翻译源语言文案从各语言模块提升至词条级别统一展示。
- **框架识别增强**：完善 react-i18next 框架的命名空间识别和插件变量花括号匹配逻辑。
- **硬编码提取健壮性**：当配置的提取路径无效时，仅弹出告警提示，不再中断整体提取流程。
- **正则匹配优化**：改进正则匹配逻辑，解决部分场景下词条 key 出现重复引用的问题。
- **AI 前缀选择增强**：优化 AI 选择 key 前缀时对扁平结构的处理逻辑，提升选择准确性。

### 功能增强
- **多余词条删除**：以翻译源语言文件为基准统计同步时，支持对“多余”类别下的词条翻译进行右键删除操作。

## [1.3.2] - 2026-03-09

### 新增
- **豆包 AI 服务**：新增豆包 AI 翻译服务支持，扩展翻译服务选择范围。
- **AI 智能前缀选择**：`writeRules.keyPrefix` 配置新增 `ai-selection` 选项，启用 AI 自动选择生成 key 的前缀。配合新增的 `writeRules.prefixCandidates` 配置，可自定义可选前缀列表，未配置时将自动识别项目中使用的前缀。

### 优化
- **智能键名风格识别**：`writeRules.keyStyle` 配置新增 `auto` 选项并设为默认值，插件会自动分析并匹配项目语言文件的 key 命名风格（大驼峰、小驼峰等）。
- **前缀选择流程精简**：优化生成 key 的前缀选择逻辑，减少不必要的交互步骤。
- **装饰策略智能化**：移除 `translationHints.decorationScope` 配置，新增 `translationHints.fullFileMaxSizeKB`，根据文件大小智能切换装饰范围（≤设定值使用全文件装饰，0 表示始终仅可见区域，-1 表示始终全文件）。
- **初始化配置写入优化**：首次运行项目初始化写入工作区配置由同步改为异步，解决多配置并发写入时 `.settings.json` 文件偶现未正常写入的问题。
- **侧边栏操作精细化**：完善树视图的操作选项，对缺少或空值的词条语言隐藏复制词条命令，改为展示复制词条名命令；同步模块标题在存在缺失翻译时开放补充缺失翻译入口。
...
### 新增
- **硬编码字符提取**：新增硬编码文本提取模块，支持对未国际化项目快速添加国际化支持，以及对已国际化项目进行补充完善。提供近二十条配置项，可通过 `ctrl/cmd + alt + e` 快捷键或 `i18nMage.extractHardcodedTexts` 命令触发。
- **YAML 语言文件支持**：新增对 YAML 格式语言文件的完整支持，扩展项目适配范围。
...
- **实时装饰更新配置**：开放 `translationHints.realtimeVisibleRangeUpdate` 配置，控制滚动时是否立即更新可见区域装饰，平衡视觉反馈与性能。
- **装饰作用范围配置**：开放 `translationHints.decorationScope` 配置，支持选择装饰应用于可见区域或整个文件，解决粘滞区域显示问题。
- **OpenAI 代理支持**：统一 ChatGPT 相关配置命名为 OpenAI，新增代理配置并修正请求地址，提升服务稳定性。
...
- **配置项清理**：移除实际作用有限的 `analysis.languageFileParser` 配置，简化设置项。
- **键盘快捷关闭**：修复预览界面支持通过 Escape 键快速关闭，提升操作效率。
